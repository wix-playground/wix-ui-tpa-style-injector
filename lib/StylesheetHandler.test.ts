...
describe('StylesheetHandler: service to interact with stylesheets in the DOM', () => {
  ...
  describe('insertRule()', () => {
    it('should create a stylesheet with unique ID if it doesnt exist on the DOM when adding css rule', () => {
      ...
      stylesheetHandler.insertRule(selector, cssRule, stylesheetId)
      const styleData = documentMock.getAddedElements()[0]

      expect(styleData.id).toBe(`${hash}-${stylesheetId}`)
      expect(styleData.sheet.rules.length).toEqual(1)
      expect(styleData.sheet.rules[0].css).toEqual(`${selector} {${cssRule}}`)
    })
    ...
    it('should replace old rules when inserting new one on the same selector', () => {
      ...
      stylesheetHandler.insertRule(selector, updatedRule, stylesheetId)

      const styleData = documentMock.getAddedElements()[0]
      expect(styleData.sheet.rules.length).toEqual(1)
      expect(styleData.sheet.rules[0].css).toEqual(`${selector} {${updatedRule}}`)
    })
    ...
    it('should properly update rules, even when they are with different selectors', () => {
      ...
      stylesheetHandler.insertRule(selector1, rule1, stylesheetId)
      stylesheetHandler.insertRule(selector2, rule2, stylesheetId)
      stylesheetHandler.insertRule(selector3, rule3, stylesheetId)

      const styleData = documentMock.getAddedElements()[0]
      expect(styleData.sheet.rules.length).toBe(3)
      expect(styleData.sheet.rules[0].css).toBe(`${selector1} {${rule1}}`)
      expect(styleData.sheet.rules[1].css).toBe(`${selector2} {${rule2}}`)
      expect(styleData.sheet.rules[2].css).toBe(`${selector3} {${rule3}}`)

      stylesheetHandler.insertRule(selector3, rule1, stylesheetId)
      stylesheetHandler.insertRule(selector1, rule2, stylesheetId)
      stylesheetHandler.insertRule(selector2, rule3, stylesheetId)

      const updatedStyleData = documentMock.getAddedElements()[0]
      expect(updatedStyleData.sheet.rules.length).toBe(3)
      expect(updatedStyleData.sheet.rules[0].css).toBe(`${selector3} {${rule1}}`)
      expect(updatedStyleData.sheet.rules[1].css).toBe(`${selector1} {${rule2}}`)
      expect(updatedStyleData.sheet.rules[2].css).toBe(`${selector2} {${rule3}}`)
    })
    ...
    it('should wrap given css with @media if its provided', () => {
      ...
      stylesheetHandler.insertRule(selector, cssRule, stylesheetId, mediaQuery)

      const stylesheet = documentMock.getAddedElements()[0].sheet
      expect(stylesheet.rules[0].css).toBe(`@media ${mediaQuery} {${selector} {${cssRule}}}`)
    })
  })
